...
def police_signup(request):
    if request.method=="POST":
        ...
        uniquepoliceid=request.POST.get('uniquepoliceid','')
...
def add_criminals(request):
    if request.method=="POST":
        ...
        criminal.save()
        update=CriminalUpdate(criminalid=criminal.criminalid,update_crime=crime)
        update.save()
...
def missing_detect(request):
    if request.method=="POST":
        ...
        for i in range(0,len(missing)):
            ...
            if True in results:
                messages.success(request,"Match Found! A mail has been sent. Thanks for helping us.")
                subject=missing[i].name+" Found"
                message=missing[i].name+ ", age "+ missing[i].age+ ", has been found by "+ name +", mobile number: " +mobile
...
def file_report(request):
    ...
    return render(request,'login/file_report.html')
...
def update_report(request):
    if request.method=="POST":
        ...
        res=Report.objects.filter(report_id=report_id)
        if len(res)!=0 and res[0].name==name:
            update=ReportUpdate(report_id=report_id, update_desc=update_desc)
            update.save()
            messages.success(request,"Report Updated Successfully")
            return redirect('Update Reports')
        else:
            messages.error(request,"Invalid Details!")
            return redirect('Update Reports')
    return render(request,'login/update_report.html')

def update_criminal(request):
    if request.method=="POST":
        criminalid=request.POST.get('criminalid','')
        name=request.POST.get('name','')
        ...
        res=Criminal.objects.filter(criminalid=criminalid)
        if len(res)!=0 and (res[0].firstname+" "+res[0].lastname)==name:
            update=CriminalUpdate(criminalid=criminalid, update_crime=update_crime, update_locations=update_locations, time=timespot)
            update.save()
            messages.success(request,"Report Updated Successfully")
            return redirect('Update Criminal')
        else:
            messages.error(request,"Invalid Details!")
            return redirect('Update Criminal')
    return render(request,'login/update_criminal.html')
